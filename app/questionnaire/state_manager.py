...
class StateManager(object):
    ...
    @staticmethod
    def save_post_date(post_data):
        if settings.EQ_SERVER_SIDE_STORAGE_TYPE == 'DATABASE':
            DatabaseStateManager.save_post_date(post_data)
        else:
            InMemoryStateManager.save_post_date(post_data)
...
class InMemoryStateManager(StateManager):
    ...
    @classmethod
    def save_post_date(cls, post_data):
        pass
...
class DatabaseStateManager(StateManager):
    ...
    def has_state():
        ...
        return POST_DATA in questionnaire_data
    ...
    def save_post_date(post_data):
        ...
        questionnaire_data[POST_DATA].append(DatabaseStateManager._convert_to_dict(post_data))
        current_user.save()
    ...
    def recover_from_post_data():
        logger.error("Recovering from post data")
        from app.questionnaire.questionnaire_manager import QuestionnaireManager
        from app.questionnaire.create_questionnaire_manager import get_schema

        logger.error("Creating questionnaire manager")
        questionnaire_manager = QuestionnaireManager.new_instance(get_schema())

        logger.error("Retrieving questionnaire data")
        questionnaire_data = current_user.get_questionnaire_data()
        if POST_DATA not in questionnaire_data:
            questionnaire_data[POST_DATA] = []

        all_post_data = questionnaire_data[POST_DATA]
        logger.error("All post data %s", all_post_data)
        location = questionnaire_manager.get_first_location()

        # basically this replays the post data in order
        for post_data in all_post_data:
            logger.error("Replaying post data %s", post_data)
            data_to_replay = DatabaseStateManager._convert_to_multi_dict(post_data)
        ...
        logger.error("Post data replayed")

        return questionnaire_manager.convert_to_state()
